...
import { Sparkles, UserPlus, CheckCircle2, ShieldCheck, Zap } from "lucide-react";
...
const Register = () => {
  ...
  return (
    <div className="min-h-screen flex bg-white">
      {/* Left Side: Value Proposition (Unique to Register) */}
      <div className="hidden lg:flex w-1/3 bg-slate-900 p-12 flex-col justify-between text-white">
        <div>
          ...
          <h2 className="text-3xl font-bold mb-8">Join 2,000+ companies scaling with SubTrack.</h2>

          <div className="space-y-6">
            <FeatureItem 
              icon={<Zap className="text-amber-400" size={20} />} 
              title="Instant Setup" 
              desc="Get your first subscription live in under 5 minutes." 
            />
            <FeatureItem 
              icon={<CheckCircle2 className="text-emerald-400" size={20} />} 
              title="Automated Dunning" 
              desc="Recover 15% more failed payments automatically." 
            />
            <FeatureItem 
              icon={<ShieldCheck className="text-indigo-400" size={20} />} 
              title="PCI-DSS Compliant" 
              desc="Security is our priority. Your data is always encrypted." 
            />
          </div>
        </div>
        ...
        <div className="bg-slate-800/50 p-6 rounded-2xl border border-slate-700">
          <p className="text-sm text-slate-300 italic">
            "SubTrack changed how we handle billing. It's the most intuitive tool we've used this year."
          </p>
          <p className="text-sm font-bold mt-3 text-white">— Sarah Chen, CEO at TechFlow</p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-2/3 flex items-center justify-center p-8 bg-slate-50/50">
        <div className="w-full max-w-md">
          <div className="mb-10">
            ...
            <p className="text-slate-500">Scale your business, not your paperwork.</p>
          </div>
...
// Small helper component for the sidebar
